...
def get_kernel(X1, X2, NA, NB, sigma=1):
    ...
    K = qml.kernels.get_local_kernels_laplacian(X1, X2, NA, NB, [sigma])[0]
    return K
...
def opt_hypers(X_train, atoms_train, y_train):
    sigmas = [0.25,0.5,0.75, 1e0, 1e1, 1e2]
    l2regs = [1e-8,1e-7]
...
if __name__ == '__main__':
    ...
    if NEW_FIT:
        ...
        for xyz_target, y_target in zip(TARGETS_XYZ, TARGETS_y):
            ...
            np.savez(f'./results/SLATM_laplace_learning_curve_sml_{target_name}.npz', train_sizes=N, all_maes_random=all_maes_random, mae_sml=mae_sml, ranking_xyz=xyzs[opt_ranking])
    
    if PLOT:

        for xyz_target in TARGETS_XYZ:
            target_name = xyz_target.split(".")[0]
            LEARNING_CURVE = np.load(f'./results/SLATM_laplace_learning_curve_sml_{target_name}.npz')
            ...
            ax.grid()
            #save figure
            fig.savefig(f'./results/SLATM_laplace_learning_curve_sml_{target_name}.png', dpi=300)

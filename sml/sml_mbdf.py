...
def train_model(X_train, atoms_train, y_train, sigma=1, l2reg=1e-9):
    ...
    K_train = qml.kernels.get_local_symmetric_kernels(X_train, atoms_train, [sigma])[0]
    print(K_train)
    print(K_train.shape)
...
def get_representations_mbdf(mols):
    Nat = np.array([len(mol.nuclear_charges) for mol in mols])
    CHARGES = np.array([(mol.nuclear_charges) for mol in mols])
    COORDINATES = np.array([(mol.coordinates) for mol in mols])
    X = generate_mbdf(CHARGES, COORDINATES, local = True)
    return X, Nat
...
if __name__ == '__main__':
    ...
    if NEW_FIT:
        ...
        for xyz_target, y_target in zip(TARGETS_XYZ, TARGETS_y):
            ...
            mols      = [qml.Compound(f"{FRAGMENTS_PATH}/{x}.xyz") for x in xyzs]
            Q_train  = [mol.nuclear_charges for mol in mols]
            ...
            Q_target = target_mol.nuclear_charges
            #Q_train = np.concatenate([Q_train, Q_target])
            #mols = np.concatenate([mols, [target_mol]])
            #rather use np.append
            
            Q_train.append(Q_target)
            mols.append(target_mol)
            
            X_train, N_train = get_representations_mbdf(mols)

            X_target, N_target = X_train[-1], N_train[-1]
            #remove last element (the target) from X, Q and N
            
            X_train, Q_train, N_train = X_train[:-1], Q_train[:-1], N_train[:-1]
            
            for i, mol_ncharges in enumerate(Q_train):
            ...
            for i, (train_index, test_index) in enumerate(kf.split(X_train)):
                ...
                for n in N:
                    min_sigma, min_l2reg = opt_hypers(X_train[train_index][:n], Q_train[train_index][:n], y_train[train_index][:n])
                    print(min_sigma, min_l2reg)
                    mae, y_pred          = train_predict_model(X_train[train_index][:n], Q_train[train_index][:n], y_train[train_index][:n], X_target, Q_target, y_target, sigma=min_sigma, l2reg=min_l2reg)
            ...
            np.savez(f'./results/mbdf_learning_curve_sml_{target_name}.npz', train_sizes=N, all_maes_random=all_maes_random, mae_sml=mae_sml, ranking_xyz=xyzs[opt_ranking])
    
    if PLOT:
        for xyz_target in TARGETS_XYZ:
            target_name = xyz_target.split(".")[0]
            LEARNING_CURVE = np.load(f'./results/mbdf_learning_curve_sml_{target_name}.npz')
            ...
            ax.grid()
            #save figure
            fig.savefig(f'./results/mbdf_learning_curve_sml_{target_name}.png', dpi=300)

...
if __name__ == "__main__":
    ...
    if NEW_FIT:
        ...
        for xyz_target, y_target in zip(TARGETS_XYZ, TARGETS_y):
            ...
            np.savez(f"testdata.npz", reps=X, labels=xyzs, ncharges=Q)
            ...
            np.savez(f"test_{target_name}.npz", rep=X_target[0], ncharges=Q_target[0])
            ...
            for n in N:
                ...
                min_sigma, min_l2reg = opt_hypers(
                    X[ranking], Q[ranking], y_train[ranking], X_target, Q_target, y_target
                )
                mae, y_pred = train_predict_model(
                    X[ranking],
                    Q[ranking],
            ...
            for i in range(CV):
                ...
                for n in N:
                    min_sigma, min_l2reg = opt_hypers(
                        X_sub[:n], Q_sub[:n], y_sub[:n], X_target, Q_target, y_target
                    )
                    mae, y_pred = train_predict_model(
                        X_sub[:n],
                        Q_sub[:n],
            ...
            np.savez(
                f"./results/learning_curve_sml_{target_name}.npz",
                train_sizes=N,
                all_maes_random=all_maes_random,
                mae_sml=mae_sml,
                ranking_xyz=xyzs[opt_ranking],
            )
    ...
    if PLOT:
        for xyz_target in TARGETS_XYZ:
            ...
            MEAN_RANDOM, STD_RANDOM = (
                np.mean(LEARNING_CURVE["all_maes_random"], axis=0) * Ha2kcal,
                np.std(LEARNING_CURVE["all_maes_random"], axis=0) * Ha2kcal,
            )
